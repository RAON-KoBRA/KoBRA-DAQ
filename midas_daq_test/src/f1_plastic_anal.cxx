...
static TH1D *hPTDCHists_raw[N_TDC_f1_PLASTIC];
...
INT F1PLASTIC_TDC_INIT(void)
{
   ...
   for (int i=0; i<N_TDC_f1_PLASTIC; i++)
   {
       //       printf("%0x\n",id);
       sprintf(name,  "F1PL-%03d", i);
       sprintf(title, "F1PL-%03d", i);
       hPTDCHists_raw[i] = h1_book<TH1D>(name, title, PTAC_N_BINS, PTAC_X_LOW, PTAC_X_HIGH);
    }
   hPXDIFFTDCHists_raw = h1_book<TH1D>("F1PL X DIFF", "F1PL X DIFF", 1000, -4000, 4000);
   hPYDIFFTDCHists_raw = h1_book<TH1D>("F1PL Y DIFF", "F1PL Y DIFF", 1000, -4000, 4000);
   hP2DTDCHists_raw = h2_book<TH2D>("F1PL 2D", "F1PL 2D plot", 1000, -4000, 4000, 1000, -4000, 4000);
...
INT F1PLASTIC_TDC(EVENT_HEADER *pheader, void *pevent)
{
   ...
   bk_size = bk_locate(pevent, BANK_NAME_F1PLASTIC, &pdata);
   if (!bk_size) {
      printf("no data\n");
      return 1;
   }
   ...
   memset(tdc_value, 0, sizeof(int32_t)*N_TDC_f1_PLASTIC);
...
//printf("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
   ...
   printf("F1PLASTIC ===============================%d\n", bk_size);
   	 ...
   	 for(int i=0; i<N_TDC_f1_PLASTIC; i++){printf("channel:%d, measure:%8f\n", i, tdc_value[i]*0.025);}
   ...
   for(int i=0; i<N_TDC_f1_PLASTIC; i++)
    {
    	 sprintf(name,  "F1PLASTIC-%03x", i);
    	 hPTDCHists_raw[i]->Fill(tdc_value[i]*f1_pl.gain, 1);
    }
