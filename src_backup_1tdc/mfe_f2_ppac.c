...
#define F1_V1290A_ADDR 0XEFFF
static cvt_V1190_data v1290a;
...
INT f2_ppac_init(int32_t BHandle)
{
	...
	printf("\n========================== Initializing V1290A board data...");

	memset( &v1290a, 0, sizeof(v1290a));

	if(!cvt_V1190_open(&v1290a, F1_V1290A_ADDR, BHandle, CVT_V1290_TYPE_A))
	{
		printf("\n Error executing cvt_V1290A_open \n");
		return FE_ERR_HW;
	}
	printf("Done\n");
	...
	addr=(F1_V1290A_ADDR<<16)|CVT_V1190_FW_REV_ADD;
	...
	//clear data
	printf("\nSending data clear...");
		if(!cvt_V1190_data_clear(&v1290a)){printf("\nError executing software clean\n");}
	printf("Done\n");
	...
	addr=(F1_V1290A_ADDR<<16)|CVT_V1190_CONTROL_ADD;
...
INT f2_ppac_exit()
{
	cvt_V1190_module_reset(&v1290a);
	cvt_V1190_close(&v1290a);
	return SUCCESS;
...
INT f2_ppac_begin(INT run_number, char *error, TRIGGER_SETTINGS *ts)
{
	...
	INT16 window_offset=ts->f2_window_offset/25;
...
//	printf("Setting_trigger_acquisition mode ...");
	...
	if(! cvt_V1190_set_trigger_matching_acquisition_mode(&v1290a, window_width, window_offset,
...
//	printf("Setting_enable_subtraction_of_trigger_time...");
	if(!cvt_V1190_write_2_micro(&v1290a, CVT_V1190_EN_SUB_TRG_OPCODE, NULL, 0))
	{
...
//	printf("Setting readout mode...");
	if(!cvt_V1190_set_readout_mode(&v1290a, TRUE, TRUE, blt_event_num))
	{
...
INT f2_ppac_check_fifo(int32_t BHandle)
{
	...
	addr = (F1_V1290A_ADDR<<16)|CVT_V1190_EVENT_STORED_ADD;
...
INT f2_ppac_read_fifo(int32_t BHandle, void *buff_tmp, int size)
{
	...
	addr = (F1_V1290A_ADDR<<16)|CVT_V1190_OUT_BUFFER_ADD;
...
int16_t nb=f1_ppac_check_fifo(BHandle);
	...
	for(i=0; i<count/4; i++)
	{
		...
		switch(data&CVT_V1190_DATA_TYPE_MSK)
		{
			case CVT_V1190_GLOBAL_HEADER:
			{
				...
				*pdata++=event_count;
				*pdata++=geo;
				printf("f2_Global_header; event_count:%d, geo:%08x\n", event_count, geo);
			} break;
			...
			case CVT_V1190_GLOBAL_TRAILER:
			{
				...
				*pdata++=trailer_wcount;
				printf("f2_Global_trailer; wcount:%08x\n", trailer_wcount);
			} break;
