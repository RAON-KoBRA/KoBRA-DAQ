...
INT f3_silicon_init(int32_t BHandle, TRIGGER_SETTINGS *ts)
{
	...
	for(i=0; i<32; i++)
	{
		addr=(F3_SILICON_ADDR<<16)|0x1080|2*i;   //32ch-->addr=(F3_SILICON_ADDR<<16)|0x1080|2*i, 16ch-->addr=(F3_SILICON_ADDR<<16)|0x1080|4*i 
	...
	ret=f3_silicon_clear_buffer(BHandle);
	printf("Done");
...
INT f3_silicon_begin(int32_t BHandle, INT run_number, char *error, TRIGGER_SETTINGS *ts)
{
	...
	reg_value=0x4980; //default 0x4880
