...
INT f2_ppac_init(int32_t BHandle)
{
	...
	if(!cvt_V1190_open(&F2_PPAC_type, F2_PPAC_ADDR, BHandle, CVT_V1290_TYPE_N))
	{
...
INT f2_ppac_begin(INT run_number, char *error, TRIGGER_SETTINGS_BEAMLINE *ts)
{
	...
	INT16 window_width=ts->f2_ppac_window_width/25;

	INT16 window_offset=0;
	if (ts->f2_ppac_window_offset>0 || ts->f2_ppac_window_offset==0){window_offset=ts->f2_ppac_window_offset/25;}
	if (ts->f2_ppac_window_offset<0){window_offset=4096-(ts->f2_ppac_window_offset/25)*-1;}

	INT16 extra_search_margin=ts->f2_ppac_extra_search_margin/25;
	INT16 reject_margin=ts->f2_ppac_reject_margin/25;
	INT blt_event_num=1;
