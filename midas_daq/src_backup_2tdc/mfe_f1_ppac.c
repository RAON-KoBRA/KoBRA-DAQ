...
INT f1_ppac_init(int32_t BHandle)
{
	...
	if(!cvt_V1190_open(&F1_PPAC_type, F1_PPAC_ADDR, BHandle, CVT_V1290_TYPE_N))
	{
...
INT f1_ppac_begin(INT run_number, char *error, TRIGGER_SETTINGS *ts)
{
	...
	if (ts->f1_window_offset>0 || ts->f1_window_offset==0){window_offset=ts->f1_window_offset/25;}
	if (ts->f1_window_offset<0){window_offset=4096-(ts->f1_window_offset/25)*-1;}
			...
			case CVT_V1190_GLOBAL_TRIGGER_TIME:
				{
					...
					if(f1_f_time>global_time){f1_time_tag+=0x8000000;}
					f1_f_time=global_time;
				#else
					UINT32 global_time=time_stamp1;
					f1_time_tag=time_stamp2;
				#endif
					*pdata++=f1_time_tag;
					*pdata++=global_time;
					printf("f1_Global_time_tag:%d, u_time:%11f\n", global_time, (f1_time_tag|global_time)*800e-9);
				} break;
