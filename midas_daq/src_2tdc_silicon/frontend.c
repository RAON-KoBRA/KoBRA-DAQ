...
INT frontend_init() {
  ...
  //VME initialization
  	...
  	if(CAENVME_Init(VMEBoard, Link, Device, &BHandle)!=cvSuccess){
  		cm_msg(MERROR, "kobra_detector", "Failed to Open CAEN V1718");
  		return FE_ERR_HW;
  	}
...
#ifdef USE_SILICON_F3
	f3silicon_buff=malloc(DATA_BUFF_SIZE);
...
INT read_trigger_event(char *pevent, INT off) {
	...
	f3_silicon_read_event(BHandle, BANK_NAME_F3SILICON, pevent, off, f3silicon_buff, DATA_BUFF_SIZE, f3silicon_data);
#endif
