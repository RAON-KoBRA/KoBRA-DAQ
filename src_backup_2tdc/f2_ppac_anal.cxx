...
static TH1D *hPX1X2DIFFTDCHists_raw;
static TH1D *hPY1Y2DIFFTDCHists_raw;
static TH2D *hPXY122DTDCHists_raw;
...
INT F2_TDC_INIT(void)
{
   ...
   hPXDIFFTDCHists_raw = h1_book<TH1D>("F2 X DIFF", "F2 X DIFF", 1000, -4000, 4000);
   hPYDIFFTDCHists_raw = h1_book<TH1D>("F2 Y DIFF", "F2 Y DIFF", 1000, -4000, 4000);
   hP2DTDCHists_raw = h2_book<TH2D>("F2 2D", "F2 2D plot", 1000, -4000, 4000, 1000, -4000, 4000);

   hPX1X2DIFFTDCHists_raw = h1_book<TH1D>("F1 X DIFF - F2 X DIFF", "F1 X DIFF - F2 X DIFF", 1000, -4000, 4000);
   hPY1Y2DIFFTDCHists_raw = h1_book<TH1D>("F1 Y DIFF - F2 Y DIFF", "F1 Y DIFF - F2 Y DIFF", 1000, -4000, 4000);
   hPXY122DTDCHists_raw = h2_book<TH2D>("F1 - F2 2D", "F1 - F2 2D plot", 1000, -4000, 4000, 1000, -4000, 4000);
...
INT F2_TDC(EVENT_HEADER *pheader, void *pevent)
{
   ...
   bk_size = bk_locate(pevent, BANK_NAME_F2FFAC, &pdata);
   if (!bk_size) {
      printf("no data\n");
      return 1;
   }
...
//printf("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
   ...
   printf("F2PPAC ===============================%d\n", bk_size);
   	 ...
   	 for(int i=0; i<N_TDC_f2_PPAC; i++){printf("channel:%d, measure:%8f\n", i, tdc_value[i]*0.025);}
...
/**/
   ...
   printf("F1F2_F1PPAC ===============================%d\n", bk_size1);
    ...
    if (tdc_value[0]+tdc_value[1]>1000 && tdc_value1[0]+tdc_value1[1]>1000) hPX1X2DIFFTDCHists_raw->Fill(tdc_value[0]-tdc_value[1]-tdc_value1[0]+tdc_value1[1],1);
    if (tdc_value[2]+tdc_value[3]>1000 && tdc_value1[2]+tdc_value1[3]>1000) hPY1Y2DIFFTDCHists_raw->Fill(tdc_value[2]-tdc_value[3]-tdc_value1[2]+tdc_value1[3],1);
    if (tdc_value[0]+tdc_value[1]>1000 && tdc_value[2]+tdc_value[3]>1000 && tdc_value1[0]+tdc_value1[1]>1000 && tdc_value1[2]+tdc_value1[3]>1000) hPXY122DTDCHists_raw->Fill(tdc_value[0]-tdc_value[1]-tdc_value1[0]+tdc_value1[1], tdc_value[2]-tdc_value[3]-tdc_value1[2]+tdc_value1[3], 1);
/**/  
